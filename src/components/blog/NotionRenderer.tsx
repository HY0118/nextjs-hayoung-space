...
export default function NotionRenderer({ blocks }: NotionRendererProps) {
    ...
    switch (type) {
      ...
      case 'quote':
        return (
          <blockquote
            key={id}
            className="border-l-4 border-primary pl-4 py-2 mb-4 italic bg-gray-50"
          >
            {value.rich_text.map((text: any) => text.plain_text).join('')}
          </blockquote>
        );
      ...
      case 'child_database': {
        ...
        return (
          <div
            key={id}
            className="my-3 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3"
          >
            <span className="font-medium text-text-primary">{title}</span>
        ...
        return (
          ...
          >
            <table className="w-full border-collapse border border-gray-300">
              <tbody>{/* 테이블 내용은 별도의 table_row 블록으로 처리됨 */}</tbody>
            </table>
          </div>
        );
  ...
  return (
    <div className="notion-content">{blocks.map((block) => renderBlock(block))}</div>
  );
}
